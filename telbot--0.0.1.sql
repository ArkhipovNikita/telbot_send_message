...
CREATE FUNCTION telbot.set_param(param varchar(100), val varchar(100)) RETURNS void AS 
$$ 
    INSERT INTO telbot.conf VALUES (param, val) ON CONFLICT (key) DO UPDATE SET value = val WHERE telbot.conf.key = param;
$$ 
LANGUAGE SQL;

CREATE FUNCTION telbot.set_chat_id(param varchar(100)) RETURNS void 
    AS $$ SELECT telbot.set_param('chat_id', param) $$
    LANGUAGE SQL;
